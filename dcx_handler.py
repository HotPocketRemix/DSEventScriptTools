...
def compress_dcx_content(content, use_ds3_header = False):
    ...
    if use_ds3_header:
        return_data += struct.pack(">IIII", 0x18, 0x24, 0x44, 0x4c)
    else:
        return_data += struct.pack(">IIII", 0x18, 0x24, 0x24, 0x2c)
...
def uncompress_dcx_content(content):
    ...
    (req_2, req_3, req_4, req_5) = struct.unpack_from(">IIII", content, offset=master_offset)
    ...
    if req_4 != 0x24 and req_4 != 0x44:
        raise ValueError("Expected DCX header int 0x24|0x44, but received " + hex(req_4))
    if req_5 != 0x2c and req_5 != 0x4c:
        raise ValueError("Expected DCX header int 0x24|0x4c, but received " + hex(req_5))
